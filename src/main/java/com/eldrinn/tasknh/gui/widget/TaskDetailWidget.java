...
import com.eldrinn.tasknh.network.DeleteTaskPacket;
import com.eldrinn.tasknh.network.RemindTaskPacket;
...
public class TaskDetailWidget extends Flow {
    ...
    public TaskDetailWidget(TaskNHGuiData data) {
        ...
        this.data = data;
        int height = TaskNHGui.getHeight();
        int innerH = height - 2 * TaskNHGui.PADDING;
        int innerW = TaskNHGui.LEFT_WIDTH - 2 * TaskNHGui.PADDING;
        ...
        if (data.createMode) {
            ...
            Task found = TaskNHClientCache.get(data.selectedTaskId);
            this.task = found;
            this.isNew = false;
            if (found != null) {
                buildForm();
            } else {
                formList.child(new TextWidget<>(t("tasknh.gui.not_found")));
            }
        } else {
            this.task = null;
            this.isNew = false;
            formList.child(new TextWidget<>(t("tasknh.gui.placeholder")));
        }
    }
    ...
    private void buildForm() {
        ...
        Flow locationHeader = Flow.row()
            .size(W, ROW_H);
        var locationLabel = new TextWidget<>(t("tasknh.gui.detail.location"));
        ...
        var showMapLabel = new TextWidget<>(t("tasknh.gui.detail.show_on_map"));
     ...
     * Returns false and keeps the old count when the text does not parse.
     */
    private static boolean applyCount(String text, IntConsumer setCount) {
        ...
        double value = result.getResult()
            .getNumberValue()
            .doubleValue();
        setCount.accept(Task.clampTrackItemCount((int) Math.round(value)));
        return true;
    }
